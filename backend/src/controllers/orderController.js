...
export const getOrderById = async (req, res) => {
  ...
  const [items] = await pool.query(
    "SELECT m.name, oi.quantity, m.price FROM OrderItem oi JOIN Menu m ON oi.item_id = m.item_id WHERE oi.order_id = ?",
    [req.params.id]
  );
...
export const getAllOrders = async (req, res) => {
  try {
    // 🗓️ Get date from query or use today
    const today = new Date().toISOString().split("T")[0];
    const date = req.query.date || today;

    // ✅ Fetch all orders for that date
    const [orders] = await pool.query(
      `SELECT o.order_id, o.order_no, o.user_id, u.name AS customer_name, 
              o.amount, o.status, o.payment_mode, o.order_date
       FROM Orders o
    ...
    // 🧾 Attach item details to each order
...
export const getUserOrders = async (req, res) => {
  try {
    ...
    // 👇 The key fix: Return the array directly — no wrapper
